    ...
    <Layout>
      ...
      <div style={s.counters}>
        {[['aberto','Abertos','#F59E0B'],['em_andamento','Em andamento','#3B82F6'],['resolvido','Resolvidos','#10B981']].map(([k,l,c]) => (
          <div key={k} style={{ ...s.counter, borderTop: `3px solid ${c}` }} onClick={() => setFiltro(filtro === k ? '' : k)}>
        ...
        {tickets.length === 0 ? (
          ...
          return (
            <div key={t.id} style={s.card}>
              ...
              {(user.role === 'facilities' || user.role === 'admin') && !['resolvido','cancelado'].includes(t.status) && (
                <div style={s.acoes}>
                  {t.status === 'aberto' && (
                    <button style={s.btnAcao} onClick={() => atualizarStatus(t.id, 'em_andamento')}>Iniciar</button>
                  )}
                  {t.status === 'em_andamento' && (
                    <button style={{ ...s.btnAcao, background: 'rgba(16,185,129,0.15)', color: '#10B981', borderColor: 'rgba(16,185,129,0.3)' }} onClick={() => atualizarStatus(t.id, 'resolvido')}>Concluir</button>
                  )}
                </div>
              )}
            </div>
...
const s = {
  ...
  counter:     { background:'var(--card)', border:'1px solid var(--border)', borderRadius:12, padding:'14px 20px', flex:'1 1 100px', cursor:'pointer', textAlign:'center' },
  ...
  card:        { background:'var(--card)', border:'1px solid var(--border)', borderRadius:14, display:'flex', overflow:'hidden' },
  ...
  acoes:       { display:'flex', alignItems:'center', padding:'0 16px', flexShrink:0 },
  btnAcao:     { background:'rgba(124,58,237,0.15)', color:'var(--purple2)', border:'1px solid rgba(124,58,237,0.3)', borderRadius:8, padding:'7px 14px', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'Sora,sans-serif', whiteSpace:'nowrap' },
  ...
  modal:       { background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:20, padding:28, width:'100%', maxWidth:480, maxHeight:'90vh', overflowY:'auto' },
  ...
  modalSub:    { fontSize:12, color:'var(--text3)', marginBottom:20, display:'flex', alignItems:'center', gap:6 },
